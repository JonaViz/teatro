...
const ObraLists = ({ obras, search, handleDelete }) => {
	const obrasList = obras?.filter((obra) => obra.name.toLowerCase().includes(search?.toLowerCase() || ''))

	return !!obrasList.length ? (
			...
			{obrasList.map((obra, index) => {
				return (
					<div key={index} className="flex min-w-fit flex-grow rounded-md bg-white drop-shadow-md">
						<img src={obra.img} className="h-36 rounded-md object-contain drop-shadow-md sm:h-48" />
						<div className="flex flex-grow flex-col justify-between p-2">
							<div>
								<p className="text-lg font-semibold sm:text-xl">{obra.name}</p>
								<p>duración : {obra.length || '-'} min.</p>
							</div>
							<button
