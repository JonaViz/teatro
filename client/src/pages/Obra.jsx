...
import ObraLists from '../components/ObraLists.jsx'
...
const Obra = () => {
	...
	const onAddObra = async (data) => {
		try {
			...
			fetchObras()
			toast.success('Obra agregada successful!', {
				position: 'top-center',
				autoClose: 2000,
				pauseOnHover: false
			})
	...
	const handleDelete = (obra) => {
		const confirmed = window.confirm(
			`¿Quieres eliminar la obra ${obra.name}, incluyendo sus espectáculos y entradas?`
		)
		if (confirmed) {
			onDeleteObra(obra._id)
		}
	}

	const onDeleteObra = async (id) => {
		try {
			const response = await axios.delete(`/obra/${id}`, {
				headers: {
					Authorization: `Bearer ${auth.token}`
				}
			})
			// console.log(response.data)
			fetchObras()
			toast.success('Obra borrada successful!', {
				position: 'top-center',
				autoClose: 2000,
				pauseOnHover: false
			})
	...
	return (
		<div className="flex min-h-screen flex-col gap-4 bg-gradient-to-br from-indigo-900 to-blue-500 pb-8 text-gray-900 sm:gap-8">
			...
			<div className="mx-4 flex h-fit flex-col gap-4 rounded-md bg-gradient-to-br from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6">
				<h2 className="text-3xl font-bold text-gray-900">Lista de Obras</h2>
				<form
					onSubmit={handleSubmit(onAddObra)}
				...
				>
					<div className="flex w-full grow flex-col flex-wrap justify-start gap-4 lg:w-auto">
						<h3 className="text-xl font-bold">Agregar Obra</h3>
						<div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center">
							<label className="text-lg font-semibold leading-5">Nombre :</label>
							<input
								type="text"
								required
						...
						<div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center">
							<label className="text-lg font-semibold leading-5">Duracion (hr.):</label>
							<input
								type="number"
								min="0"
								max="20"
						...
						<div>
							<div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center">
								<label className="text-lg font-semibold leading-5">Duracion (min.):</label>
								<input
									type="number"
									min="0"
									max="2000"
					...
					<div className="flex w-full flex-col gap-4 lg:w-auto lg:flex-row">
						...
						<button
							...
							type="submit"
							disabled={isAddingObra}
						>
							{isAddingObra ? 'Procesando...' : 'ADD +'}
						</button>
					</div>
				</form>
				<div className="relative drop-shadow-sm">
					...
					<input
						...
						placeholder="Search obra"
						{...register('search')}
					/>
				</div>
				{isFetchingObrasDone ? (
					<ObraLists obras={obras} search={watch('search')} handleDelete={handleDelete} />
				) : (
					<Loading />
				)}
			</div>
