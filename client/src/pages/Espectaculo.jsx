...
const Espectaculo = () => {
	...
	const fetchEspectaculo = async (data) => {
		try {
			let response
			if (auth.role === 'admin') {
				response = await axios.get(`/espectaculo/user/${id}`, {
					headers: {
	...
	const column = espectaculo?.sala?.seatPlan.column
	let colNumber = []
	for (let k = 1; k <= column; k++) {
		colNumber.push(k)
	}

	const isPast = new Date(espectaculo.espectaculo) < new Date()
	const filteredSeats = espectaculo?.seats?.filter((seat) => {
		return (
		...
		<div className="flex min-h-screen flex-col gap-4 bg-gradient-to-br from-indigo-900 to-blue-500 pb-8 sm:gap-8">
			...
			<div className="mx-4 h-fit rounded-lg bg-gradient-to-br from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6">
				{espectaculo.espectaculo ? (
					<>
						<EspecaculoDetails espectaculo={espectaculo} showDeleteBtn={true} fetchEspectaculo={fetchEspectaculo} />
						...
						{auth.role === 'admin' && (
							<>
								<h2 className="mt-4 text-2xl font-bold">Asientos reservados</h2>
								<div className="mt-2 flex gap-2 rounded-md bg-gradient-to-br from-indigo-100 to-white p-4">
									<div className="flex grow flex-col">
										...
										<Select
											value={filterRow}
											options={Array.from(new Set(espectaculo?.seats.map((seat) => seat.row)))
												.sort((a, b) => {
													const rowA = a.row
													const rowB = b.row
													if (rowA === rowB) {
									...
									<div className="flex grow flex-col">
										<h4 className="text-lg font-bold text-gray-800">Columna</h4>
										<Select
											value={filterColumn}
											options={Array.from(new Set(espectaculo?.seats.map((seat) => seat.number)))
												.sort((a, b) => {
													return a - b
												})
												.map((value) => ({
