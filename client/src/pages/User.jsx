...
const User = () => {
	...
	const handleDelete = (data) => {
		const confirmed = window.confirm(`¿Quieres eliminar al usuario $ {data.username}?`)
		if (confirmed) {
			onDeleteUser(data)
		}
	}
	...
	return (
		<div className="flex min-h-screen flex-col gap-4 bg-gradient-to-br from-indigo-900 to-blue-500 pb-8 text-gray-900 sm:gap-8">
			...
			<div className="mx-4 flex h-fit flex-col gap-2 rounded-lg bg-gradient-to-br from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6">
							...
							return (
								<Fragment key={index}>
									...
									<div className="border-t-2 border-indigo-200 px-2 py-1">
										...
										>
											Ver {user.tickets.length} Tickets
				...
				{ticketsUser && (
					<>
						<h2 className="mt-4 text-2xl font-bold text-gray-900">Ver las entradas de {ticketsUser}</h2>
						{tickets.length === 0 ? (
							<p className="text-center">Este usuario aún no ha comprado ninguna entrada.</p>
						) : (
